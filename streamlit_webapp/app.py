...
st_1.title("🚀Table Detection & Structure Recognition ")
with st_2:
...
class TableExtractionPipeline:
    ...
    def crop_tables(
        ...
        for p, (xmin, ymin, xmax, ymax) in zip(prob, boxes.tolist()):
            ...
            img= cv2.imread(f"temp/{image_path.name}")
            xmin, ymin, xmax, ymax = int(xmin), int(ymin), int(xmax), int(ymax)
            cropped_image = img[ymin:ymax, xmin:xmax]
            ctr += 1
            cropped_img_path = re.sub("((.png)|(.jp(e)?g)|(.bmp))","", image_path.name)
            cropped_img_name = f"{cropped_img_path}_cropped_img_{ctr}.png"
            cv2.imwrite(f"cropped_images/{cropped_img_name}", cropped_image) 
    ...
    def create_dataframe(self, c3, cell_ocr_res: list, max_cols: int, max_rows: int):
        ...
        c3.download_button(
            "Export as CSV", csv, "file.csv", "text/csv", key="download-csv-" + numkey
        )
    ...
    async def start_process(
        ...
        if bboxes_scaled.nelement() == 0:
            st.write("No table found in the PDF's page image")
            return ""
...
if __name__ == "__main__":
    ...
    st.warning("⚠️ Errors may occur due to poor image quality/OCR performance.")

    te = TableExtractionPipeline()
    # for img in image_list:
    if img_name is not None:
        with st.spinner("Processing... 💫"):
            with open(f"temp/{img_name.name}","wb") as f:
